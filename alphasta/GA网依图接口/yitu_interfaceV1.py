...
offset = 1000
for count in range(9754641//offset + 1):
    data_list = []
    time.sleep(2)
    params_str = {'extra_fields':['custom_field_1'], 'condition':{'repository_ids':['13']}, 'order':{'timestamp':-1}, 'start':count * offset, 'limit':offset}
    response_dict = eval(requests.post(url + 'condition/query_repository', headers = headers, data = json.dumps(params_str)).text)
    if response_dict['rtn'] == 0:
        for row in range(len(response_dict['results'])):
            # print(response_dict['results'][row])
            face_image_id = response_dict['results'][row]['face_image_id']
            # picture_uri = pic_url_prefix + str(base64.b64encode(response_dict['results'][row]['picture_uri'].encode('utf8')), encoding='utf8')
            face_image_uri = pic_url_prefix + str(base64.b64encode(response_dict['results'][row]['face_image_uri'].encode('utf8')), encoding='utf8')
            person_id = response_dict['results'][row]['person_id']
            name = response_dict['results'][row]['name']
        ...
        try:
            ...
            # print(res)
            cursor.prepare('insert into LIGH.YITU13 (image_id, image_url, person_id, uname) values (:1, :2, :3, :4)')
            cursor.executemany(None, data_list)
            db.commit()
            print('成功插入' + str(offset) + '条')
        except Exception as e:
            print(e, str(count))

    else:
        print(response_dict, str(count))
